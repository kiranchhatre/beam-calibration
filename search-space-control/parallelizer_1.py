...
for k in range(len(counter)): # per stage start x=(number of parallel pass 7 or 4) and 1(for bookkeeping) procs
    while True:
        ...
        if file_text == 'write '+str(k)+' done':
            break 
    parallel_passes = list(range(len(glob.glob(beam+'/test/input/sf-light/urbansim-10k_*.conf'))))
    for m in range(len(parallel_passes)):               
        #multiprocessing.log_to_stderr(logging.DEBUG)
        p = Process(target=fire_BEAM, args=(m))
        p.start()
        BEAM_procs.append(p)
    with open(beam+"/firecue.txt", "w") as text_file: 
        text_file.write('fire '+str(k)+' done')    
    q = Process(target=bookkeep, args=(bookkeeping_iters[k])) 
    q.start()
